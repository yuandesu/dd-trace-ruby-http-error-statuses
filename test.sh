...
echo "   /forbidden (403):     span.status=0 → APM error ❌"
echo "   /unprocessable (422): span.status=0 → APM error ❌"
echo ""
echo " after (DD_TRACE_HTTP_SERVER_ERROR_STATUSES=403,422,500-599):"
echo "   /forbidden (403):     span.status=1 → APM error ✅"
echo "   /unprocessable (422): span.status=1 → APM error ✅"
echo "   ※ error.type/error.message なし → Error Tracking ❌"
echo "=========================================="
